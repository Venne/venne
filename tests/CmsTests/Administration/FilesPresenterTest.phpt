...
class FilesPresenterTest extends AdministrationCase
{
	...
	{
		$response = $this->getResponse('Cms:Admin:Files', 'GET');
		Assert::true($response instanceof TextResponse);
		Assert::true($response->getSource() instanceof ITemplate);
		$dom = $this->getDom($response);

		$this->assertCssContain($dom, 'Files and attachements', 'h1');
		$this->assertXpathContain($dom, 'Dashboard', '//div[@id="snippet--header"]/ul/li/a');
		$this->assertXpathContain($dom, 'Files and attachements', '//div[@id="snippet--header"]/ul/li[@class="active"]');
	}
