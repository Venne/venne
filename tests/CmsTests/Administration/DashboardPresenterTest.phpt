...
class DashboardPresenterTest extends AdministrationCase
{
	...
	{
		$this->helper->createResponse('Cms:Admin:Dashboard', 'GET')
			->type('Nette\Application\Responses\TextResponse')
			->getTemplate()->type('Nette\Templating\ITemplate')
			->getDom()
			->contains('Dashboard', 'h1')
			->xpathContains('Dashboard', '//div[@id="snippet--header"]/ul/li/a')
			->contains('Logged in user', '#snippet--content h2')
			->xpathContains('Administrator', '//*[@id="snippet--content"]//h4');
	}
